...
const sleeperIndex =
  buildSleeperNameIndex(sleeperPlayers);

const rankingByFantasyProsId =
  new Map(
...
const players = projectionRows
  .map((rawPlayer) => {
    const player =
      normalizeFantasyProsPlayer(
        rawPlayer
      );

    const ranking =
      rankingByFantasyProsId.get(
        String(player.fantasyprosId)
      ) || {};
...
const output = {
  ...
  providers: [
    ...
    },
    {
      ...
      status: "live",
      role:
        "Player identity and league data",
    },
  ],
